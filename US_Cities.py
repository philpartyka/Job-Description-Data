'''
This file contains the list of US cities that are able to be used as a parameter for TheMuse's api.
The commented out code below is how the list was created.  
Since the list only needs to be created once the code is commented out.
'''


# from bs4 import BeautifulSoup
# import requests
# import re

# url = 'https://www.themuse.com/developers/api/v2'
# response = requests.get(url)
# data = response.text
# soup = BeautifulSoup(data, 'html.parser')

# location_html = soup.find(class_='form-group expansion',attrs={"data-argument-name":"location"})
# locations=[]
# for loc in location_html.find_all('label'):
#     locations.append(loc.text)

# pattern=re.compile('.*, [A-Z]{2}$')
# US_Cities=list(filter(pattern.match,locations))



US_Cities = ['Aberdeen, MD',
 'Aberdeen, SD',
 'Aberdeen, WA',
 'Abilene, TX',
 'Accord, NY',
 'Acton, CA',
 'Ada, OK',
 'Addison, IL',
 'Addison, TX',
 'Adelanto, CA',
 'Adrian, MI',
 'Advance, NC',
 'Agawam, MA',
 'Agoura Hills, CA',
 'Ahwatukee, AZ',
 'Aiken, SC',
 'Akron, OH',
 'Alameda, CA',
 'Alamogordo, NM',
 'Albany, GA',
 'Albany, NY',
 'Albany, OR',
 'Albemarle, NC',
 'Albert Lea, MN',
 'Albuquerque, NM',
 'Alcoa, TN',
 'Alexandria, LA',
 'Alexandria, VA',
 'Alice, TX',
 'Allen Park, MI',
 'Allentown, PA',
 'Allen, TX',
 'Alliance, OH',
 'Alma, MI',
 'Alpena, MI',
 'Alpharetta, GA',
 'Altamont, OR',
 'Alton, IL',
 'Altoona, PA',
 'Altus, OK',
 'Amarillo, TX',
 'Amboy, IL',
 'American Canyon, CA',
 'American Fork, UT',
 'Americus, GA',
 'Ames, IA',
 'Amherst, MA',
 'Amsterdam, NY',
 'Anaheim, CA',
 'Anchorage, AK',
 'Anderson, IN',
 'Anderson, SC',
 'Andover, CT',
 'Andover, MA',
 'Angleton, TX',
 'Anna Maria, FL',
 'Annapolis Junction, MD',
 'Annapolis, MD',
 'Ann Arbor, MI',
 'Ansonia, CT',
 'Antelope, CA',
 'Anthem, AZ',
 'Antioch, CA',
 'Antioch, TN',
 'Apache Junction, AZ',
 'Apopka, FL',
 'Appleton, WI',
 'Apple Valley, CA',
 'Apple Valley, MN',
 'Arcata, CA',
 'Ardmore, OK',
 'Arley, AL',
 'Arlington Heights, IL',
 'Arlington, TX',
 'Arlington, VA',
 'Arvin, CA',
 'Asbury Park, NJ',
 'Ashburn, VA',
 'Asheboro, NC',
 'Asheville, NC',
 'Ashford, CT',
 'Ashland, OH',
 'Ashland, OR',
 'Ashland, VA',
 'Ashtabula, OH',
 'Aspen, CO',
 'Astoria, NY',
 'Atascadero, CA',
 'Atascocita, TX',
 'Athens, GA',
 'Athens, OH',
 'Atlanta, GA',
 'Atlanta, IL',
 'Atlantic City, NJ',
 'Attleboro, MA',
 'Atwater, CA',
 'Auburn, AL',
 'Auburn Hills, MI',
 'Auburn, MA',
 'Auburn, ME',
 'Auburn, NY',
 'Auburn, WA',
 'Augusta, GA',
 'Aurora, CO',
 'Aurora, IL',
 'Austin, MN',
 'Austin, TX',
 'Aventura, FL',
 'Avon, CT',
 'Avon, IN',
 'Avon, MA',
 'Avon, OH',
 'Bad Axe, MI',
 'Badger, AK',
 'Bakersfield, CA',
 'Baltimore, MD',
 'Banning, CA',
 'Bar Harbor, ME',
 'Barkhamsted, CT',
 'Barrington, RI',
 'Barstow, CA',
 'Barstow Heights, CA',
 'Bartlesville, OK',
 'Basking Ridge, NJ',
 'Batavia, NY',
 'Batesville, IN',
 'Bath, ME',
 'Baton Rouge, LA',
 'Battle Creek, MI',
 'Battle Ground, WA',
 'Bay City, MI',
 'Bayou Cane, LA',
 'Bayside, CA',
 'Baytown, TX',
 'Beachwood, OH',
 'Beacon Falls, CT',
 'Beaufort, NC',
 'Beaumont, CA',
 'Beaumont, TX',
 'Beaver Creek, CO',
 'Beaver Dam, WI',
 'Beaverton, MI',
 'Beaverton, OR',
 'Bedford, MA',
 'Bedford, NH',
 'Bedminster, NJ',
 'Bel Air North, MD',
 'Belleair Bluffs, FL',
 'Belle Glade, FL',
 'Belle Mead, NJ',
 'Belleville, IL',
 'Belleville, MI',
 'Belleville, NJ',
 'Bellevue, NE',
 'Bellevue, WA',
 'Bellingham, MA',
 'Bellingham, WA',
 'Belmont, CA',
 'Belmont, MI',
 'Beloit, WI',
 'Belvidere, IL',
 'Bemidji, MN',
 'Bend, OR',
 'Benton, AR',
 'Bentonville, AR',
 'Berkeley, CA',
 'Berkeley Heights, NJ',
 'Berlin, CT',
 'Berlin, DE',
 'Berwyn, PA',
 'Bethany, CT',
 'Bethel, CT',
 'Bethesda, MD',
 'Bethlehem, CT',
 'Bethlehem, PA',
 'Bettendorf, IA',
 'Beverly Hills, CA',
 'Biddeford, ME',
 'Big Pine Key, FL',
 'Big Spring, TX',
 'Billerica, MA',
 'Billingsley, AL',
 'Billings, MT',
 'Biloxi, MS',
 'Binghamton, NY',
 'Birmingham, AL',
 'Birmingham, MI',
 'Bismarck, ND',
 'Bixby, OK',
 'Blaine, MN',
 'Blaine, WA',
 'Bloomfield, CT',
 'Bloomfield Hills, MI',
 'Bloomfield, NJ',
 'Bloomington, IL',
 'Bloomington, IN',
 'Bloomington, MN',
 'Blountville, TN',
 'Blue Ash, OH',
 'Bluffton, SC',
 'Blythe, CA',
 'Boardman, OH',
 'Boca Raton, FL',
 'Boise, ID',
 'Bolingbrook, IL',
 'Bolton, CT',
 'Bonita Springs, FL',
 'Bonney Lake, WA',
 'Boone, NC',
 'Boonton Township, NJ',
 'Bordentown, NJ',
 'Bossier City, LA',
 'Bostonia, CA',
 'Boston, MA',
 'Boston, NY',
 'Bothell, WA',
 'Boulder City, NV',
 'Boulder, CO',
 'Bourbonnais, IL',
 'Bowling Green, KY',
 'Bowling Green, OH',
 'Bow, WA',
 'Boxborough, MA',
 'Boynton Beach, FL',
 'Bozeman, MT',
 'Bozrah, CT',
 'Bradenton, FL',
 'Bradley, IL',
 'Braintree, MA',
 'Branchburg, NJ',
 'Brandon, FL',
 'Brandywine, MD',
 'Branford, CT',
 'Branson, MO',
 'Brawley, CA',
 'Breinigsville, PA',
 'Bremerton, WA',
 'Brenham, TX',
 'Brentwood, CA',
 'Brentwood, TN',
 'Brewer, ME',
 'Bridgeport, CT',
 'Bridgeport, TX',
 'Bridgeton, NJ',
 'Bridgewater, CT',
 'Bridgewater, MA',
 'Bridgewater, NJ',
 'Brigham City, UT',
 'Brighton, CO',
 'Brighton, MI',
 'Bristol, CT',
 'Bristol, TN',
 'Broken Arrow, OK',
 'Brookfield, CT',
 'Brookfield, WI',
 'Brookings, SD',
 'Brooklyn, CT',
 'Brooklyn, NY',
 'Brooklyn Park, MN',
 'Brooksville, FL',
 'Broomall, PA',
 'Broomfield, CO',
 'Brownsburg, IN',
 'Brownstown Township, MI',
 'Brownsville, TX',
 'Brownwood, TX',
 'Brunswick, GA',
 'Brunswick, OH',
 'Brushy Creek, TX',
 'Bryant, AR',
 'Buchanan, MI',
 'Buckeye, AZ',
 'Buckhead, GA',
 'Buena, NJ',
 'Buenaventura Lakes, FL',
 'Buffalo Grove, IL',
 'Buffalo, MN',
 'Buffalo, NY',
 'Buford, GA',
 'Bullhead City, AZ',
 'Burbank, CA',
 'Burkesville, KY',
 'Burlington, CT',
 'Burlington, IA',
 'Burlington, KY',
 'Burlington, MA',
 'Burlington, NC',
 'Burlington, VT',
 'Burlington, WA',
 'Burlington, WI',
 'Burnsville, MN',
 'Butte, MT',
 'Butte-Silver Bow (Balance), MT',
 'Byron Center, MI',
 'Cabazon, CA',
 'Cabot, AR',
 'Cadillac, MI',
 'Caldwell, ID',
 'Caledonia, MI',
 'Calexico, CA',
 'Calhoun, GA',
 'California, MD',
 'Calumet City, IL',
 'Camarillo, CA',
 'Camas, WA',
 'Cambridge, MA',
 'Camden, AR',
 'Camden, NJ',
 'Cameron Park, CA',
 'Campbell, CA',
 'Canaan, CT',
 'Cañon City, CO',
 'Canonsburg, PA',
 'Canterbury, CT',
 'Canton, CT',
 'Canton, GA',
 'Cantonment, FL',
 'Canton, MI',
 'Canton, OH',
 'Canyon Lake, TX',
 'Cape Coral, FL',
 'Cape Girardeau, MO',
 'Cape Haze, FL',
 'Cape May Courthouse, NJ',
 'Cape May Court House, NJ',
 'Cape May, NJ',
 'Capitola, CA',
 'Capitol Heights, MD',
 'Carbondale, IL',
 'Carleton, MI',
 'Carlisle, PA',
 'Carlsbad, CA',
 'Carlsbad, NM',
 'Carmel, IN',
 'Caro, MI',
 'Carpinteria, CA',
 'Carroll, IA',
 'Carrollton, GA',
 'Carrollton, KY',
 'Carrollton, TX',
 'Cartersville, GA',
 'Cary, IL',
 'Cary, NC',
 'Casa Grande, AZ',
 'Casper, WY',
 'Castaic, CA',
 'Castle Hayne, NC',
 'Cathedral City, CA',
 'Catonsville, MD',
 'Cedar City, UT',
 'Cedar Knolls, NJ',
 'Cedar Park, TX',
 'Cedar Point, NC',
 'Cedar Rapids, IA',
 'Center Valley, PA',
 'Centralia, WA',
 'Central Valley, NY',
 'Cerritos, CA',
 'Chagrin Falls, OH',
 'Chambersburg, PA',
 'Champaign, IL',
 'Chandler, AZ',
 'Chanhassen, MN',
 'Channelview, TX',
 'Chantilly, VA',
 'Chapel Hill, NC',
 'Chaplin, CT',
 'Charleston, IL',
 'Charleston, SC',
 'Charleston, WV',
 'Charlevoix, MI',
 'Charlotte, NC',
 'Charlottesville, VA',
 'Chattanooga, TN',
 'Chehalis, WA',
 'Cherry Creek, CO',
 'Cherry Hill, NJ',
 'Chesapeake, VA',
 'Cheshire, CT',
 'Chester, CT',
 'Chesterfield, MO',
 'Chestertown, MD',
 'Chestnut Hill, MA',
 'Chevy Chase, MD',
 'Cheyenne, WY',
 'Chicago, IL',
 'Chickasha, OK',
 'Chico, CA',
 'Chicopee, MA',
 'Chino Hills, CA',
 'Chocowinity, NC',
 'Chowchilla, CA',
 'Chula Vista, CA',
 'Cibolo, TX',
 'Cincinnati, OH',
 'Citrus Heights, CA',
 'Clackamas, OR',
 'Claremont, NC',
 'Claremore, OK',
 'Clarksburg, WV',
 'Clarksdale, MS',
 'Clarkston, MI',
 'Clarksville, MD',
 'Clarksville, TN',
 'Clayton, MO',
 'Clayton, NC',
 'Clearlake, CA',
 'Clearwater, FL',
 'Cleburne, TX',
 'Clemson, SC',
 'Clermont, FL',
 'Clermont, KY',
 'Cleveland, OH',
 'Cleveland, TN',
 'Clifton, CO',
 'Clifton Heights, PA',
 'Clifton, NJ',
 'Clinton, CT',
 'Clinton, IA',
 'Clinton, IN',
 'Clinton, SC',
 'Clinton Township, MI',
 'Clinton, UT',
 'Clive, IA',
 'Cloverdale, CA',
 'Cloverdale, OR',
 'Clovis, NM',
 'Coachella, CA',
 'Coalinga, CA',
 'Cocoa, FL',
 "Coeur d'Alene, ID",
 'Colchester, CT',
 'Coldwater, MI',
 'Colebrook, CT',
 'College Park, GA',
 'College Station, TX',
 'Collegeville, PA',
 'Collierville, TN',
 'Colonial Beach, VA',
 'Colonial Heights, VA',
 'Colorado Springs, CO',
 'Columbia, CT',
 'Columbia, MD',
 'Columbia, MO',
 'Columbia, SC',
 'Columbia, TN',
 'Columbus, GA',
 'Columbus, IN',
 'Columbus, KS',
 'Columbus, MS',
 'Columbus, NE',
 'Columbus, OH',
 'Columbus, TX',
 'Commerce City, CO',
 'Commerce Township, MI',
 'Compton, CA',
 'Concord, CA',
 'Concord, MA',
 'Concord, NC',
 'Concord, NH',
 'Concrete, WA',
 'Conshohocken, PA',
 'Contoocook, NH',
 'Converse, TX',
 'Conway, AR',
 'Conway, SC',
 'Conyers, GA',
 'Cookeville, TN',
 'Coon Rapids, MN',
 'Coos Bay, OR',
 'Coppell, TX',
 'Copperas Cove, TX',
 'Coral Gables, FL',
 'Coral Springs, FL',
 'Coraopolis, PA',
 'Corcoran, CA',
 'Cordova, TN',
 'Corinth, TX',
 'Cornwall, CT',
 'Corona, CA',
 'Coronado, CA',
 'Corpus Christi, TX',
 'Corsicana, TX',
 'Corte Madera, CA',
 'Cortez, FL',
 'Cortland, NY',
 'Corvallis, OR',
 'Costa Mesa, CA',
 'Coventry, CT',
 'Coventry, RI',
 'Covington, KY',
 'Covington, TN',
 'Covington, WA',
 'Cranberry Township, PA',
 'Cranston, RI',
 'Crawfordsville, IN',
 'Crestview, FL',
 'Crestwood, IL',
 'Crestwood, KY',
 'Crocker Park, OH',
 'Crofton, MD',
 'Cromwell, CT',
 'Crystal City, VA',
 'Crystal Lake, IL',
 'Crystal River, FL',
 'Cudahy, WI',
 'Cullman, AL',
 'Culver City, CA',
 'Cumberland, MD',
 'Cumberland, ME',
 'Cupertino, CA',
 'Cuyahoga Falls, OH',
 'Cypress, CA',
 'Cypress, TX',
 'Dahlgren, VA',
 'Dakota Dunes, SD',
 'Dallas, OR',
 'Dallas, TX',
 'Dalton, GA',
 'Daly City, CA',
 'Damascus, MD',
 'Danbury, CT',
 'Dania Beach, FL',
 'Danvers, MA',
 'Danville, IL',
 'Danville, KY',
 'Darien, CT',
 'Davie, FL',
 'Davis, CA',
 'Davison, MI',
 'Dawsonville, GA',
 'Daytona Beach, FL',
 'Dayton, NV',
 'Dayton, OH',
 'Dearborn Heights, MI',
 'Dearborn, MI',
 'Decatur, AL',
 'Decatur, GA',
 'Decatur, IL',
 'Decorah, IA',
 'Dedham, MA',
 'Deep River, CT',
 'Deerfield Beach, FL',
 'Deerfield, IL',
 'Deer Park, NY',
 'Deer Park, TX',
 'Defiance, OH',
 'DeKalb, IL',
 'Del Amo, CA',
 'DeLand, FL',
 'Delano, CA',
 'Delaware, OH',
 'Delray Beach, FL',
 'Del Rio, TX',
 'Denison, TX',
 'Dennis, MA',
 'Denton, MD',
 'Denton, TX',
 'Denver, CO',
 'Derby, CT',
 'Des Moines, IA',
 'Des Moines, WA',
 'DeSoto, TX',
 'Destin, FL',
 'Detroit, MI',
 'Devens, MA',
 'Dewitt, MI',
 'Dexter, MI',
 'DFW, TX',
 'Dickinson, ND',
 'Dickson, TN',
 'Dinuba, CA',
 'Dixiana, AL',
 'Dixon, CA',
 'Dixon, IL',
 'Dodge City, KS',
 'Donna, TX',
 'Doral, FL',
 'Dothan, AL',
 'Douglas, AZ',
 'Douglasville, GA',
 'Dover, DE',
 'Dover, PA',
 "Downer's Grove, IL",
 'Downers Grove, IL',
 'Draper, UT',
 'Duarte, CA',
 'Dublin, GA',
 'Dublin, OH',
 'Dubuque, IA',
 'Dulles, VA',
 'Duluth, GA',
 'Duluth, MN',
 'Dumas, TX',
 'Duncan, OK',
 'Dunedin, FL',
 'Durango, CO',
 'Durant, OK',
 'Durham, CT',
 'Durham, NC',
 'Dyersburg, TN',
 'Eagan, MN',
 'Eagle Pass, TX',
 'Eagle River, AK',
 'East Aurora, NY',
 'East Concord, NH',
 'East Florence, AL',
 'Eastford, CT',
 'East Granby, CT',
 'East Haddam, CT',
 'East Hampton, CT',
 'East Hampton, NY',
 'East Hanover, NJ',
 'East Hartford, CT',
 'East Hartland, CT',
 'East Haven, CT',
 'East Hemet, CA',
 'East Hill-Meridian, WA',
 'East Lansing, MI',
 'East Longmeadow, MA',
 'East Lyme, CT',
 'Easton, CT',
 'Easton, MA',
 'Easton, MD',
 'East Stroudsburg, PA',
 'East Windsor, CT',
 'Eatontown, NJ',
 'Eau Claire, WI',
 'Eddystone, PA',
 'Eden, NC',
 'Eden Prairie, MN',
 'Edgewater, FL',
 'Edina, MN',
 'Edison, NJ',
 'Edmond, OK',
 'Edwardsville, IL',
 'El Cajon, CA',
 'El Centro, CA',
 'El Dorado, AR',
 'El Dorado Hills, CA',
 'Elgin, IL',
 'Elizabeth City, NC',
 'Elizabethtown, KY',
 'Elk Grove, CA',
 'Elk Grove Village, IL',
 'Elkhorn, WI',
 'Elko, NV',
 'Elkridge, MD',
 'Elk River, MN',
 'Ellensburg, WA',
 'Ellenton, FL',
 'Ellicott City, MD',
 'Ellington, CT',
 'Ellsworth, ME',
 'Elmira, NY',
 'Eloy, AZ',
 'El Paso, TX',
 'El Reno, OK',
 'El Segundo, CA',
 'Emeryville, CA',
 'Emigrant Gap, CA',
 'Emporia, KS',
 'Enchanted Hills, NM',
 'Encino, CA',
 'Endicott, NY',
 'Enfield, CT',
 'Englewood Cliffs, NJ',
 'Englewood, CO',
 'Enid, OK',
 'Ennis, TX',
 'Enon, OH',
 'Enterprise, AL',
 'Erie, CO',
 'Erie, PA',
 'Escanaba, MI',
 'Escondido, CA',
 'Essex, CT',
 'Estero, FL',
 'Eugene, OR',
 'Eureka, CA',
 'Eustis, FL',
 'Evans, CO',
 'Evanston, IL',
 'Evansville, IN',
 'Everett, WA',
 'Evergreen Park, IL',
 'Exton, PA',
 'Fairbanks, AK',
 'Fairfax, NC',
 'Fairfax, VA',
 'Fairfield, CA',
 'Fairfield, CT',
 'Fairfield Heights, IN',
 'Fairhaven, MA',
 'Fairhope, AL',
 'Fairlawn, OH',
 'Fair Oaks, CA',
 'Fair Play, SC',
 'Fallbrook, CA',
 'Fall City, WA',
 'Fall River, MA',
 'Falls Church, VA',
 'Falmouth, MA',
 'Fargo, ND',
 'Faribault, MN',
 'Farmington, CT',
 'Farmington Hills, MI',
 'Farmington, NM',
 'Farmington, UT',
 'Fayetteville, AR',
 'Fayetteville, NC',
 'Federal Way, WA',
 'Fenton, MI',
 'Fenton, MO',
 'Fernley, NV',
 'Fillmore, CA',
 'Findlay, OH',
 'Firestone, CO',
 'Fishers, IN',
 'Fishkill, NY',
 'Flagstaff, AZ',
 'Flemington, NJ',
 'Flint, MI',
 'Florence, AL',
 'Florence, AZ',
 'Florence, KY',
 'Florence, OR',
 'Florence, SC',
 'Florham Park, NJ',
 'Florissant, MO',
 'Foley, AL',
 'Folsom, CA',
 'Fond du Lac, WI',
 'Fontana-On-Geneva Lake, WI',
 'Forest Lake, MN',
 'Forest Lakes, AZ',
 'Forney, TX',
 'Fort Atkinson, WI',
 'Fort Collins, CO',
 'Fort Dodge, IA',
 'Fort Gordon, GA',
 'Fort Gratiot, MI',
 'Fort Lauderdale, FL',
 'Fort Leonard Wood, MO',
 'Fort Myers, FL',
 'Fort Pierce, FL',
 'Fort Smith, AR',
 'Fortuna Foothills, AZ',
 'Fort Walton Beach, FL',
 'Fort Washington, PA',
 'Fort Wayne, IN',
 'Fort Worth, TX',
 'Foster City, CA',
 'Fountain Hills, AZ',
 'Four Corners, FL',
 'Four Corners, OR',
 'Framingham, MA',
 'Frankfort, IN',
 'Frankfort, KY',
 'Franklin, CT',
 'Franklin, GA',
 'Franklin, OH',
 'Franklin, TN',
 'Frankllin, GA',
 'Frederick, MD',
 'Frederickson, WA',
 'Freehold, NJ',
 'Freemont, CA',
 'Freeport, IL',
 'Fremont, CA',
 'Fremont, MI',
 'Fremont, NE',
 'Fremont, OH',
 'Fremont, WI',
 'Fresno, CA',
 'Frisco, TX',
 'Fruitland Park, FL',
 'Fulton, MD',
 'Fuquay-Varina, NC',
 'Gadsden, AL',
 'Gainesville, FL',
 'Gainesville, GA',
 'Gainesville, TX',
 'Gaithersburg, MD',
 'Galesburg, IL',
 'Gallup, NM',
 'Galt, CA',
 'Galveston, TX',
 'Gambrills, MD',
 'Garden City, KS',
 'Garden City, NY',
 'Garden City, RI',
 'Gardiner, ME',
 'Gardiner, NY',
 'Garland, TX',
 'Gastonia, NC',
 'Gatesville, TX',
 'Gautier, MS',
 'Gaylord, MI',
 'Georgetown, TX',
 'Germantown, TN',
 'Gettysburg, PA',
 'Gillette, WY',
 'Gilroy, CA',
 'Gladys, VA',
 'Glassboro, NJ',
 'Glastonbury, CT',
 'Glen Burnie, MD',
 'Glendale, AZ',
 'Glendale, CA',
 'Glendale Heights, IL',
 'Glendale, WI',
 'Glen Ellyn, IL',
 'Gloucester, MA',
 'Gloucester, VA',
 'Gloversville, NY',
 'Godfrey, IL',
 'Golden, CO',
 'Golden Valley, MN',
 'Goldsboro, NC',
 'Goleta, CA',
 'Goose Creek, SC',
 'Gordonville, TX',
 'Goshen, CT',
 'Goshen, IN',
 'Grafton, WI',
 'Graham, WA',
 'Granby, CT',
 'Grand Blanc, MI',
 'Grand Forks, ND',
 'Grand Island, FL',
 'Grand Island, NE',
 'Grand Junction, CO',
 'Grand Ledge, MI',
 'Grand Prairie, TX',
 'Grand Rapids, MI',
 'Granite Bay, CA',
 'Grants Pass, OR',
 'Grayling, MI',
 'Great Bend, KS',
 'Great Falls, MT',
 'Great Neck, NY',
 'Greeley, CO',
 'Green Bay, WI',
 'Greenbelt, MD',
 'Green Cove Springs, FL',
 'Green Creek, NJ',
 'Greendale, WI',
 'Greeneville, TN',
 'Greenfield, CA',
 'Greenfield, IN',
 'Greenfield, MA',
 'Greenfield, WI',
 'Greenlawn, NY',
 'Greensboro, NC',
 'Green Valley, AZ',
 'Greenville, AL',
 'Greenville, MI',
 'Greenville, MS',
 'Greenville, NC',
 'Greenville, SC',
 'Greenville, TX',
 'Greenville, VA',
 'Greenwich, CT',
 'Greenwood, MS',
 'Greenwood, SC',
 'Greenwood Village, CO',
 'Griffin, GA',
 'Griswold, CT',
 'Grosse Pointe Woods, MI',
 'Groton, CT',
 'Grove City, OH',
 'Groveland, CA',
 'Groves, TX',
 'Guilford, CT',
 'Gulfport, MS',
 'Gunnison, CO',
 'Gurnee, IL',
 'Hackensack, NJ',
 'Haddam, CT',
 'Hagerstown, MD',
 'Hamden, CT',
 'Hamilton, NJ',
 'Hamilton, NY',
 'Hammond, IN',
 'Hammond, LA',
 'Hampton Bays, NY',
 'Hampton, CT',
 'Hanford, CA',
 'Hannibal, MO',
 'Hanover, NH',
 'Hanover, PA',
 'Harker Heights, TX',
 'Harlingen, TX',
 'Harrisburg, PA',
 'Hartford, CT',
 'Hartland, CT',
 'Harwinton, CT',
 'Hastings, MN',
 'Hastings, NE',
 'Hattiesburg, MS',
 'Hauppauge, NY',
 'Havelock, NC',
 'Haverhill, MA',
 'Hawthorne, CA',
 'Hayesville, OR',
 'Hays, KS',
 'Hayward, CA',
 'Hazelwood, MO',
 'Hazleton, PA',
 'Heathrow, FL',
 'Hebron, CT',
 'Helena, AL',
 'Helena, MT',
 'Hemet, CA',
 'Henderson, NV',
 'Hendersonville, TN',
 'Hereford, TX',
 'Hermiston, OR',
 'Hermitage, PA',
 'Hernando, MS',
 'Herndon, VA',
 'Hesperia, CA',
 'Hibbing, MN',
 'Hickory Hills, IL',
 'Hickory, NC',
 'Higganum, CT',
 'Highlands Ranch, CO',
 'High Point, NC',
 'Hill AFB, UT',
 'Hillsboro, OR',
 'Hillsdale, MI',
 'Hilo, HI',
 'Hilton Head Island, SC',
 'Hilton Head, SC',
 'Hinesville, GA',
 'Hingham, MA',
 'Hobbs, NM',
 'Hoboken, NJ',
 'Hoffman Estates, IL',
 'Hohenwald, TN',
 'Holland, MI',
 'Hollister, CA',
 'Holly, MI',
 'Holly Springs, NC',
 'Hollywood, MD',
 'Holmdel, NJ',
 'Holyoke, MA',
 'Homestead, FL',
 'Homewood, AL',
 'Homewood, IL',
 'Honolulu, HI',
 'Hopkinsville, KY',
 'Horizon City, TX',
 'Horn Lake, MS',
 'Horsham, PA',
 'Hot Springs, AR',
 'Hot Springs National Park, AR',
 'Houghton, MI',
 'Houma, LA',
 'Houston, TX',
 'Howe, IN',
 'Howell, MI',
 'Hudson, NH',
 'Hudsonville, MI',
 'Huntersville, NC',
 'Huntington Beach, CA',
 'Huntington, IN',
 'Huntington Park, CA',
 'Huntington Station, NY',
 'Huntington, WV',
 'Huntsville, AL',
 'Huntsville, TX',
 'Hunt Valley, MD',
 'Hurricane, UT',
 'Hurst, TX',
 'Hutchinson, KS',
 'Hutto, TX',
 'Hyannis, MA',
 'Idaho Falls, ID',
 'Idyllwild, CA',
 'Immokalee, FL',
 'Imperial, CA',
 'Independence, MO',
 'Independence, OH',
 'Indianapolis, IN',
 'Indian Land, SC',
 'Indianola, IA',
 'Indio, CA',
 'Inwood, WV',
 'Iowa City, IA',
 'Iron River, MI',
 'Ironville, KY',
 'Ironwood, MI',
 'Irvine, CA',
 'Irving, CT',
 'Irving, TX',
 'Iselin, NJ',
 'Itasca, IL',
 'Ithaca, NY',
 'Ixonia, WI',
 'Jackson Hole, WY',
 'Jackson, MI',
 'Jackson, MS',
 'Jackson, TN',
 'Jacksonville, AL',
 'Jacksonville, AR',
 'Jacksonville, FL',
 'Jacksonville, IL',
 'Jacksonville, NC',
 'Jamestown, ND',
 'Jamestown, NY',
 'Jamul, CA',
 'Janesville, WI',
 'Jasper, IN',
 'Jefferson City, MO',
 'Jefferson, GA',
 'Jefferson, OH',
 'Jeffersonville, IN',
 'Jenison, MI',
 'Jenkintown, PA',
 'Jericho, NY',
 'Jersey City, NJ',
 'Johnson City, TN',
 'Johnston, RI',
 'Johnstown, PA',
 'Joliet, IL',
 'Jonesboro, AR',
 'Joplin, MO',
 'Junction City, KS',
 'Juneau, AK',
 'Jupiter, FL',
 'Kahului, HI',
 'Kalamazoo, MI',
 'Kalispell, MT',
 'Kankakee, IL',
 'Kannapolis, NC',
 'Kansas City, KS',
 'Kansas City, MO',
 'Kapolei, HI',
 'Katy, TX',
 'Kearney, NE',
 'Kearny, NJ',
 'Keene, NH',
 'Keizer, OR',
 'Kenilworth, NJ',
 'Kennesaw, GA',
 'Kenosha, WI',
 'Kent, CT',
 'Kent, OH',
 'Kent, WA',
 'Kentwood, MI',
 'Ketchikan, AK',
 'Key West, FL',
 'Kīhei, HI',
 'Kilbourne, IL',
 'Killingly, CT',
 'Killingworth, CT',
 'Kingman, AZ',
 'King of Prussia, PA',
 'King Salmon, AK',
 'Kingsland, GA',
 'Kingsport, TN',
 'Kingston, NY',
 'Kinston, NC',
 'Kiowa, OK',
 'Kirksville, MO',
 'Kiryas Joel, NY',
 'Kissimmee, FL',
 'Kittery, ME',
 'Klamath Falls, OR',
 'Knoxville, TN',
 'Kokomo, IN',
 'Kuna, ID',
 'Laconia, NH',
 'La Conner, WA',
 'Lady Lake, FL',
 'Lafayette, CA',
 'Lafayette, IN',
 'Lafayette, LA',
 'La Grange, GA',
 'La Jolla, CA',
 'Lake Bluff, IL',
 'Lake Charles, LA',
 'Lake Elmo, MN',
 'Lake Havasu City, AZ',
 'Lakehills, TX',
 'Lakeland, FL',
 'Lake Mary, FL',
 'Lake Orion, MI',
 'Lake Oswego, OR',
 'Lake Success, NY',
 'Lake Tahoe, CA',
 'Lakeville, MN',
 'Lakewood, NJ',
 'Lakewood Ranch, FL',
 'Lakewood, WA',
 'Lake Worth, FL',
 'Lake Worth, TX',
 'Lake Zurich, IL',
 'Lambertville, MI',
 'La Mirada, CA',
 'Lancaster, OH',
 'Lancaster, PA',
 'Landover, MD',
 'Lanham, MD',
 'Lansing, MI',
 'Lansing, NC',
 'Lapeer, MI',
 'Laplace, LA',
 'La Porte, IN',
 'LaPorte, IN',
 'La Quinta, CA',
 'Laramie, WY',
 'Laredo, TX',
 'Largo, FL',
 'Las Cruces, NM',
 'Las Vegas, NV',
 'Lathrop, CA',
 'Laurel, MD',
 'Laurel, MS',
 'Laurinburg, NC',
 'Lawrence, KS',
 'Lawrence, MA',
 'Lawrence Township, NJ',
 'Lawrenceville, NJ',
 'Lawton, OK',
 'Leadwood Town, MO',
 'League City, TX',
 'Leavenworth, KS',
 'Leavenworth, WA',
 'Leawood, KS',
 'Leawood, MO',
 'Lebanon, CT',
 'Lebanon, IN',
 'Lebanon, NH',
 'Lebanon, NJ',
 'Lebanon, OR',
 'Lebanon, PA',
 'Lebanon, TN',
 'Ledyard, CT',
 'Leesburg, FL',
 'Leesburg, VA',
 "Lee's Summit, MO",
 'Lehigh Acres, FL',
 'Lehi, UT',
 'Leisure City, FL',
 'Leland, NC',
 'Lenexa, KS',
 'Lenhartsville, PA',
 'Lenoir, NC',
 'Lewiston, ID',
 'Lewiston Orchards, ID',
 'Lewisville, TX',
 'Lexington, KY',
 'Lexington, MA',
 'Lexington, NC',
 'Liberal, KS',
 'Liberty Township, OH',
 'Lima, OH',
 'Lincoln, NE',
 'Lincolnshire, IL',
 'Linden, MI',
 'Lindon, UT',
 'Linthicum Heights, MD',
 'Lisbon, CT',
 'Lisle, IL',
 'Litchfield, CT',
 'Lithia Springs, GA',
 'Lititz, PA',
 'Little Rock, AR',
 'Littlestown, PA',
 'Littleton, CO',
 'Littleton, NC',
 'Livermore, CA',
 'Livingston, NJ',
 'Livonia, MI',
 'Lockhart, TX',
 'Lockport, NY',
 'Logansport, IN',
 'Logan, UT',
 'Loganville, GA',
 'Lompoc, CA',
 'Lone Tree, CO',
 'Long Beach, CA',
 'Long Beach, MS',
 'Long Branch, TX',
 'Long Island, NY',
 'Long Key, FL',
 'Longmeadow, MA',
 'Longmont, CO',
 'Longview, WA',
 'Los Alamitos, CA',
 'Los Angeles, CA',
 'Los Banos, CA',
 'Los Lunas, NM',
 'Louisville, KY',
 'Loveland, CO',
 'Lowell, MA',
 'Lubbock, TX',
 'Ludlow, MA',
 'Lumberton, NC',
 'Lutz, FL',
 'Lyme, CT',
 'Lynnfield, MA',
 'Lynnwood, WA',
 'Macedonia, OH',
 'Mackinaw City, MI',
 'Macomb, IL',
 'Macomb, MI',
 'Macon, GA',
 'Madera, CA',
 'Madison, CT',
 'Madison Heights, MI',
 'Madisonville, KY',
 'Madison, WI',
 'Mahwah, NJ',
 'Maitland, FL',
 'Makakilo City, HI',
 'Makakilo, HI',
 'Malibu, CA',
 'Malvern, PA',
 'Manchester, CT',
 'Manchester, NH',
 'Manhasset, NY',
 'Manhattan Beach, CA',
 'Manhattan, KS',
 'Manhattan, NY',
 'Manheim, PA',
 'Manitowoc, WI',
 'Mankato, MN',
 'Mansfield, CT',
 'Mansfield, OH',
 'Mansfield, TX',
 'Maple Grove, MN',
 'Maple Valley, WA',
 'Maplewood, MN',
 'Marana, AZ',
 'Marblehead, OH',
 'Marco Island, FL',
 'Marfa, TX',
 'Maricopa, AZ',
 'Marietta, GA',
 'Marina, CA',
 'Marin City, CA',
 'Marin County, CA',
 'Marion, IL',
 'Marion, IN',
 'Marion, OH',
 'Marlborough, CT',
 'Marlborough, MA',
 'Marlton, NJ',
 'Marmora, NJ',
 'Marquette, MI',
 'Marshalltown, IA',
 'Marshfield, WI',
 'Maryland Heights, MO',
 'Marysville, MI',
 'Marysville, OH',
 'Maryville, TN',
 'Mashantucket, CT',
 'Mason City, IA',
 'Mason, OH',
 'Massena, NY',
 'Mastic, NY',
 'Matawan, NJ',
 'Mattawan, MI',
 'Matthews, NC',
 'Mattoon, IL',
 'Maumee, OH',
 'Mays Landing, NJ',
 'McAlester, OK',
 'McAllen, TX',
 'McDonough, GA',
 'McHenry, IL',
 'Mckeesport, PA',
 'McKees Rocks, PA',
 'McKinleyville, CA',
 'McLean, VA',
 'McMinnville, OR',
 'Meads, KY',
 'Mead Valley, CA',
 'Mechanicsburg, PA',
 'Medford, MA',
 'Medford, OR',
 'Medina, OH',
 'Melbourne, FL',
 'Melville, NY',
 'Memphis, TN',
 'Mendota, MN',
 'Menifee, CA',
 'Menlo Park, CA',
 'Menomonee Falls, WI',
 'Menomonie, WI',
 'Mentor, OH',
 'Merced, CA',
 'Meriden, CT',
 'Meridian, MS',
 'Merrick Park, FL',
 'Merrillville, IN',
 'Merrimack, NH',
 'Merrimac, MA',
 'Merritt Island, FL',
 'Mesa, AZ',
 'Mesquite, NV',
 'Mesquite, TX',
 'Miami Beach, FL',
 'Miami, FL',
 'Miami Springs, FL',
 'Michigan City, IN',
 'Middleburg, VA',
 'Middlebury, CT',
 'Middlefield, CT',
 'Middleton, WI',
 'Middletown, CT',
 'Middletown, DE',
 'Middletown, NY',
 'Middletown, RI',
 'Midland, MI',
 'Midland, TX',
 'Midvale, UT',
 'Midway, FL',
 'Milford, CT',
 'Milford, MI',
 'Mililani Town, HI',
 'Milledgeville, GA',
 'Millville, NJ',
 'Milpitas, CA',
 'Milton, DE',
 'Milton, WI',
 'Milwaukee, WI',
 'Minneapolis, MN',
 'Minnetonka, MN',
 'Minooka, IL',
 'Minot, ND',
 'Mishawaka, IN',
 'Mission Viejo, CA',
 'Missoula, MT',
 'Missouri City, TX',
 'Mitchell, SD',
 'Mitchellville, MD',
 'Mobile, AL',
 'Moccasin, CA',
 'Mocksville, NC',
 'Modesto, CA',
 'Monee, IL',
 'Monroe, CT',
 'Monroe, GA',
 'Monroe, LA',
 'Monroe, MI',
 'Monroe, NC',
 'Monroe, OH',
 'Monroe, WA',
 'Monterey, CA',
 'Montgomery, AL',
 'Montgomery, TX',
 'Montrose, CO',
 'Montvale, NJ',
 'Montville, CT',
 'Moorestown, NJ',
 'Morgan Hill, CA',
 'Morganton, NC',
 'Morganville, NJ',
 'Morris, CT',
 'Morristown, NJ',
 'Morristown, TN',
 'Morrisville, NC',
 'Morrow, GA',
 'Moscow, ID',
 'Moses Lake, WA',
 'Mountain Home, AR',
 'Mountainside, NJ',
 'Mountain View, CA',
 'Mount Clemens, MI',
 'Mount Laurel, NJ',
 'Mount Pleasant, MI',
 'Mount Prospect, IL',
 'Mount Vernon, IL',
 'Mount Vernon, OH',
 'Mt Laurel, NJ',
 'Mt. Pleasant, SC',
 'Mt. Vernon, WA',
 'Muncie, IN',
 'Murfreesboro, TN',
 'Murphy, TX',
 'Murray, KY',
 'Murrieta, CA',
 'Murrysville, PA',
 'Muscatine, IA',
 'Muskegon, MI',
 'Muskogee, OK',
 'Mustang, OK',
 'Myrtle Beach, SC',
 'Nampa, ID',
 'Nantucket, MA',
 'Napa, CA',
 'Naperville, IL',
 'Naples, FL',
 'Narvon, PA',
 'Nashua, NH',
 'Nashville, TN',
 'Natchez, MS',
 'Natchitoches, LA',
 'Natick, MA',
 'Naugatuck, CT',
 'Navarre, FL',
 'Needham, MA',
 'Neenah, WI',
 'Newark, DE',
 'Newark, NJ',
 'Newark, OH',
 'New Baltimore, MI',
 'New Bedford, MA',
 'Newberg, OR',
 'New Berlin, WI',
 'New Bern, NC',
 'New Britain, CT',
 'New Brunswick, NJ',
 'Newburgh, NY',
 'Newbury Park, CA',
 'Newburyport, MA',
 'New Canaan, CT',
 'New Carlisle, IN',
 'New Castle, IN',
 'New Castle, PA',
 'New Century, KS',
 'New Fairfield, CT',
 'New Hartford, CT',
 'New Haven, CT',
 'New Holland, PA',
 'New Hope, PA',
 'New Hudson, MI',
 'New Iberia, LA',
 'Newington, CT',
 'New Kensington, PA',
 'New London, CT',
 'New Milford, CT',
 'Newnan, GA',
 'New Orleans, LA',
 'New Oxford, PA',
 'New Philadelphia, OH',
 'Newport Beach, CA',
 'Newport, MI',
 'Newport, NC',
 'Newport News, VA',
 'New Port Richey, FL',
 'Newport, WA',
 'New Rochelle, NY',
 'New Smyrna Beach, FL',
 'Newton, IA',
 'Newton, KS',
 'Newtown, CT',
 'Newtown, PA',
 'New York, NY',
 'Niagara Falls, NY',
 'Niles, MI',
 'Nipomo, CA',
 'Noblesville, IN',
 'Nogales, AZ',
 'Nokomis, FL',
 'Norcross, GA',
 'Norfolk, CT',
 'Norfolk, NE',
 'Norfolk, VA',
 'Norman, OK',
 'North Adams, MA',
 'North Andover, MA',
 'North Branford, CT',
 'Northbrook, IL',
 'North Canaan, CT',
 'North Charleston, SC',
 'North Chicopee, MA',
 'Northfield, MN',
 'North Franklin, CT',
 'North Haven, CT',
 'North Kingstown, RI',
 'North Miami, FL',
 'North Myrtle Beach, SC',
 'North Ogden, UT',
 'North Platte, NE',
 'Northport, AL',
 'North Reading, MA',
 'North Riverside, IL',
 'North Smithfield, RI',
 'North Stonington, CT',
 'Northville, MI',
 'Norwalk, CT',
 'Norwalk, OH',
 'Norwell, MA',
 'Norwich, CT',
 'Norwich, NY',
 'Norwich, VT',
 'Norwood, MA',
 'Nottingham, MD',
 'Novato, CA',
 'Novi, MI',
 'Oakbrook, IL',
 'Oak Brook, IL',
 'Oak Creek, WI',
 'Oakdale, MN',
 'Oak Harbor, WA',
 'Oakland, CA',
 'Oakland Park, FL',
 'Oak Lawn, IL',
 'Oakley, CA',
 'Oak Park, IL',
 'Oak Park, MI',
 'Oak Ridge, TN',
 'Ocala, FL',
 'Ocean Acres, NJ',
 'Oceanside, CA',
 'Ocean View, NJ',
 'Odessa, FL',
 'Odessa, TX',
 "O'Fallon, IL",
 "O'Fallon, MO",
 'Ogden, UT',
 'Okeechobee, FL',
 'Oklahoma City, OK',
 'Old Jamestown, MO',
 'Old Lyme, CT',
 'Old Saybrook, CT',
 'Olive Branch, MS',
 'Olmsted, OH',
 'Olney, MD',
 'Olympia, WA',
 'Omaha, NE',
 'Onalaska, WI',
 'Oneonta, NY',
 'Ontario, CA',
 'Opelika, AL',
 'Opelousas, LA',
 'Orangeburg, NY',
 'Orange, CA',
 'Orange, CT',
 'Orange Park, FL',
 'Orangevale, CA',
 'Orcutt, CA',
 'Oregon House, CA',
 'Oregon, OH',
 'Orem, UT',
 'Orlando, FL',
 'Orland Park, IL',
 'Ormond Beach, FL',
 'Oroville, CA',
 'Oshkosh, WI',
 'Oswego, NY',
 'Otsego, MN',
 'Ottawa, IL',
 'Ottumwa, IA',
 'Overland Park, KS',
 'Owatonna, MN',
 'Owensboro, KY',
 'Owings Mills, MD',
 'Oxford, AL',
 'Oxford, CT',
 'Oxford, MS',
 'Oxford, NC',
 'Oxford, OH',
 'Oxnard, CA',
 'Ozark, MO',
 'Pace, FL',
 'Pacheco, CA',
 'Pacific Grove, CA',
 'Paducah, KY',
 'Pahrump, NV',
 'Paicines, CA',
 'Palatine, IL',
 'Palm Bay, FL',
 'Palm Beach, FL',
 'Palm Beach Gardens, FL',
 'Palm Coast, FL',
 'Palmdale, CA',
 'Palm Desert, CA',
 'Palmer, MA',
 'Palmetto, FL',
 'Palm Harbor, FL',
 'Palm Springs, CA',
 'Palo Alto, CA',
 'Pampa, TX',
 'Panama City, FL',
 'Paradise, CA',
 'Paragould, AR',
 'Paramount, CA',
 'Paramus, NJ',
 'Park City, KY',
 'Park City, UT',
 'Parkersburg, WV',
 'Parkland, WA',
 'Park Ridge, IL',
 'Parlier, CA',
 'Parsippany, NJ',
 'Pasadena, CA',
 'Pasadena, MD',
 'Pascagoula, MS',
 'Paso Robles, CA',
 'Patterson, CA',
 'Payson, AZ',
 'Payson, UT',
 'Peabody, MA',
 'Peachtree City, GA',
 'Peekskill, NY',
 'Pelham, AL',
 'Pembroke Pines, FL',
 'Pendleton, OR',
 'Pennington, NJ',
 'Pennsauken, NJ',
 'Pensacola, FL',
 'Peoria, AZ',
 'Peoria, IL',
 'Perris, CA',
 'Perry, GA',
 'Petaluma, CA',
 'Petoskey, MI',
 'Pewaukee, WI',
 'Philadelphia, PA',
 'Phillipsburg, KS',
 'Phoenix, AZ',
 'Pinckney, MI',
 'Pine Bluff, AR',
 'Pinehurst, NC',
 'Pinellas Park, FL',
 'Piqua, OH',
 'Piscataway, NJ',
 'Pittsburg, CA',
 'Pittsburgh, PA',
 'Pittsburg, KS',
 'Pittsfield, MA',
 'Plainfield, CT',
 'Plainfield, IL',
 'Plainfield, IN',
 'Plainview, NY',
 'Plainview, TX',
 'Plainville, CT',
 'Plainwell, MI',
 'Plano, TX',
 'Plantation, FL',
 'Plant City, FL',
 'Plattsburgh, NY',
 'Playa Vista, CA',
 'Pleasant Grove, UT',
 'Pleasanton, CA',
 'Pleasant Prairie, WI',
 'Plum, PA',
 'Plymouth, CT',
 'Plymouth, MA',
 'Plymouth Meeting, PA',
 'Plymouth, MI',
 'Plymouth, MN',
 'Plymouth, PA',
 'Plymouth, WI',
 'Pocatello, ID',
 'Pocomoke City, MD',
 'Poinciana, FL',
 'Point, TX',
 'Pomfret, CT',
 'Pompano Beach, FL',
 'Ponca City, OK',
 'Ponce Inlet, FL',
 'Ponte Vedra Beach, FL',
 'Pontiac, MI',
 'Poplar Bluff, MO',
 'Portage, MI',
 'Port Angeles, WA',
 'Port Charlotte, FL',
 'Porterville, CA',
 'Port Huron, MI',
 'Portland, CT',
 'Portland, ME',
 'Portland, OR',
 'Port Republic, NJ',
 'Port Richey, FL',
 'Portsmouth, NH',
 'Portsmouth, OH',
 'Port St. Lucie, FL',
 'Port Washington, NY',
 'Post Falls, ID',
 'Pottsville, PA',
 'Poughkeepsie, NY',
 'Poway, CA',
 'Prairieville, LA',
 'Prescott, AR',
 'Prescott, AZ',
 'Prescott Valley, AZ',
 'Presque Isle, ME',
 'Preston, CT',
 'Princeton, FL',
 'Princeton, NJ',
 'Prospect, CT',
 'Providence, RI',
 'Provo, UT',
 'Pueblo, CO',
 'Pueblo West, CO',
 'Pulaski, NY',
 'Pullman, WA',
 'Purchase, NY',
 'Putnam, CT',
 'Puyallup, WA',
 'Quantico, VA',
 'Queen Creek, AZ',
 'Queensbury, NY',
 'Queens, NY',
 'Quinby, VA',
 'Quincy, IL',
 'Quincy, MA',
 'Quincy, WA',
 'Radcliff, KY',
 'Radnor, PA',
 'Raeford, NC',
 'Rahway, NJ',
 'Raleigh, NC',
 'Ramona, CA',
 'Rancho Penasquitos, CA',
 'Rancho San Diego, CA',
 'Randleman, NC',
 'Rapid City, SD',
 'Raritan, NJ',
 'Raymond, NH',
 'Raymore, MO',
 'Raynham, MA',
 'Reading, PA',
 'Redding, CA',
 'Redding, CT',
 'Redford, MI',
 'Redford Township, MI',
 'Redlands, CA',
 'Redmond, OR',
 'Redmond, WA',
 'Redondo Beach, CA',
 'Red Wing, MN',
 'Redwood City, CA',
 'Reedley, CA',
 'Reedsville, PA',
 'Rehoboth Beach, DE',
 'Reno, NV',
 'Research Triangle Park, NC',
 'Reston, VA',
 'Revere, MA',
 'Rexburg, ID',
 'Reynoldsburg, OH',
 'Richardson, TX',
 'Richards, TX',
 'Richfield, MN',
 'Richfield, OH',
 'Richland Hills, TX',
 'Richland, WA',
 'Richmond Heights, OH',
 'Richmond, IN',
 'Richmond, KY',
 'Richmond, MI',
 'Richmond, TX',
 'Richmond, VA',
 'Richmond West, FL',
 'Ridgecrest, CA',
 'Ridgefield, CT',
 'Ridgefield Park, NJ',
 'Rio Rancho, NM',
 'Ripon, CA',
 'River Falls, WI',
 'Riverside, CA',
 'Riverview, FL',
 'Riverwoods, IL',
 'Riviera Beach, FL',
 'Roanoke Rapids, NC',
 'Roanoke, VA',
 'Robbinsville, NJ',
 'Rochester Hills, MI',
 'Rochester, MA',
 'Rochester, MN',
 'Rochester, NH',
 'Rochester, NY',
 'Rockaway, NJ',
 'Rockford, IL',
 'Rock Hill, SC',
 'Rock Island, IL',
 'Rockland, MA',
 'Rockledge, FL',
 'Rocklin, CA',
 'Rock Springs, WY',
 'Rockton, IL',
 'Rockville, MD',
 'Rocky Hill, CT',
 'Rocky Mount, NC',
 'Rogers, AR',
 'Rogers, MN',
 'Rohnert Park, CA',
 'Rolla, MO',
 'Rolling Meadows, IL',
 'Rome, GA',
 'Rome, NY',
 'Romeoville, IL',
 'Rosamond, CA',
 'Roseburg, OR',
 'Rosemont, IL',
 'Roseville, CA',
 'Roseville, MI',
 'Roseville, MN',
 'Roswell, GA',
 'Roswell, NM',
 'Rotterdam, NY',
 'Round Rock, TX',
 'Roxboro, NC',
 'Roxbury, CT',
 'Roxbury, NJ',
 'Royal Oak, MI',
 'Roy, UT',
 'Russellville, AR',
 'Ruston, LA',
 'Rutland, VT',
 'Saco, ME',
 'Sacramento, CA',
 'Sag Harbor, NY',
 'Saginaw, MI',
 'Sahuarita, AZ',
 'Saint Charles, MD',
 'Saint Charles, MO',
 'Saint Cloud, FL',
 'Saint George, UT',
 'Saint Joseph, MO',
 'Saint Michael, MN',
 'Saint Paul, MN',
 'Salem, CT',
 'Salem, NH',
 'Salem, OR',
 'Salina, KS',
 'Salinas, CA',
 'Saline, MI',
 'Salisbury, CT',
 'Salisbury, MD',
 'Salisbury, NC',
 'Salome, AZ',
 'Salt Lake City, UT',
 'San Angelo, TX',
 'San Antonio, TX',
 'San Bernadino, CA',
 'San Carlos, CA',
 'San Clemente, CA',
 'San Diego, CA',
 'San Dimas, CA',
 'Sandusky, OH',
 'Sandy Springs, GA',
 'Sandy, UT',
 'Sanford, FL',
 'Sanford, ME',
 'Sanford, NC',
 'San Francisco, CA',
 'San Jacinto, CA',
 'San Jose, CA',
 'San Leandro, CA',
 'San Luis, AZ',
 'San Luis Obispo, CA',
 'San Marcos, CA',
 'San Marcos, TX',
 'San Mateo, CA',
 'San Rafael, CA',
 'San Ramon, CA',
 'Santa Ana, CA',
 'Santa Barbara, CA',
 'Santa Clara, CA',
 'Santa Clarita, CA',
 'Santa Cruz, CA',
 'Santa Fe, NM',
 'Santa Fe Springs, CA',
 'Santa Maria, CA',
 'Santa Monica, CA',
 'San Tan Valley, AZ',
 'Santa Paula, CA',
 'Santa Rosa, CA',
 'Santee, CA',
 'Sapulpa, OK',
 'Sarasota, FL',
 'Saratoga Springs, NY',
 'Saulsbury, TN',
 'Sault Ste. Marie, MI',
 'Savage, MN',
 'Savannah, GA',
 'Scarsdale, NY',
 'Schaumburg, IL',
 'Schenectady, NY',
 'Schofield Barracks, HI',
 'Scotland, CT',
 'Scottsbluff, NE',
 'Scottsboro, AL',
 'Scottsdale, AZ',
 'Scottsville, VA',
 'Scranton, PA',
 'Searcy, AR',
 'Seaside, OR',
 'Seattle, WA',
 'Seaview, WA',
 'Sebastian, FL',
 'Sebring, FL',
 'Secaucus, NJ',
 'Sedalia, MO',
 'Selma, AL',
 'Selma, CA',
 'Seminole, FL',
 'Severna Park, MD',
 'Sevierville, TN',
 'Seymour, CT',
 'Seymour, IN',
 'Shafter, CA',
 'Shalimar, FL',
 'Sharon, CT',
 'Shartlesville, PA',
 'Shawnee, OK',
 'Sheboygan, WI',
 'Shelby Charter Township, MI',
 'Shelby, NC',
 'Shelbyville, IN',
 'Shelbyville, KY',
 'Shelbyville, TN',
 'Shelton, CT',
 'Sheridan, AR',
 'Sheridan, WY',
 'Sherman, CT',
 'Sherman Oaks, CA',
 'Shirley, MA',
 'Shirley, NY',
 'Shiro, TX',
 'Shoreview, MN',
 'Short Hills, NJ',
 'Show Low, AZ',
 'Shreveport, LA',
 'Sidney, OH',
 'Sierra Vista, AZ',
 'Sikeston, MO',
 'Silicon Valley, CA',
 'Siloam Springs, AR',
 'Silver Creek, WA',
 'Silverdale, WA',
 'Silver Lake, WI',
 'Silver Spring, MD',
 'Simsbury, CT',
 'Sioux City, IA',
 'Sioux Falls, SD',
 'Skillman, NJ',
 'Skokie, IL',
 'Sleepy Hollow, NY',
 'Slidell, LA',
 'Smithton, PA',
 'Smyrna, GA',
 'Sneads Ferry, NC',
 'Socorro Mission Number 1 Colonia, TX',
 'Socorro, TX',
 'Soledad, CA',
 'Solon, OH',
 'Somers, CT',
 'Somerset, NJ',
 'Somerton, AZ',
 'Somerville, MA',
 'Southampton, NY',
 'South Bel Air, MD',
 'Southbury, CT',
 'South Dennis, MA',
 'South Elgin, IL',
 'Southfield, MI',
 'South Florida, FL',
 'Southgate, MI',
 'South Hampton, NH',
 'South Haven, MI',
 'South Hill, WA',
 'Southington, CT',
 'South Jordan, UT',
 'Southlake, TX',
 'South Lyon, MI',
 'South Ogden, UT',
 'Southport, IL',
 'South Portland, ME',
 'South San Francisco, CA',
 'South Vineland, NJ',
 'South Windsor, CT',
 'South Yuba City, CA',
 'Spanaway, WA',
 'Sparks, NV',
 'Spartanburg, SC',
 'Spokane, WA',
 'Sprague, CT',
 'Springdale, AR',
 'Springfield, IL',
 'Springfield, MA',
 'Springfield, MO',
 'Springfield, OH',
 'Springfield, TN',
 'Springfield, VA',
 'Spring Hill, FL',
 'Spring Hill, TN',
 'Spring House, PA',
 'Spring, TX',
 'Stafford, CT',
 'Stafford, VA',
 'St. Albans City, VT',
 'Stamford, CT',
 'Starkville, MS',
 'State College, PA',
 'Staten Island, NY',
 'Statesboro, GA',
 'Statesville, NC',
 'St. Augustine, FL',
 'St. Charles, IL',
 'St Clair, MI',
 'St. Clair, MI',
 'St. Clair Shores, MI',
 'St. Cloud, MN',
 'Sterling, CT',
 'Sterling Heights, MI',
 'Sterling, IL',
 'Sterling, VA',
 'Steubenville, OH',
 'Stevenson Ranch, CA',
 'Stevens Point, WI',
 'Stevensville, MD',
 'Stillwater, OK',
 'St. Inigoes, MD',
 'St James City, FL',
 'St. Joseph, MI',
 'St. Louis, MO',
 'St. Marys, GA',
 'Stockton, CA',
 'Stone Ridge, NY',
 'Stonington, CT',
 'Stoughton, MA',
 'Stow, OH',
 'St. Petersburg, FL',
 'St. Peters, MO',
 'Stratford, CT',
 'Strongsville, OH',
 'Stuart, FL',
 'Sturbridge, MA',
 'Sturgeon Bay, WI',
 'Sturtevant, WI',
 'Suffield, CT',
 'Suffolk, VA',
 'Sugar Valley, GA',
 'Suisun, CA',
 'Sulphur, LA',
 'Summersville, WV',
 'Summerville, SC',
 'Summit, NJ',
 'Sumter, SC',
 'Sun City, AZ',
 'Sun City, CA',
 'Sun City Center, FL',
 'Sunnyside, WA',
 'Sunnyvale, CA',
 'Sunrise, FL',
 'Surprise, AZ',
 'Susanville, CA',
 'Suwanee, GA',
 'Swainsboro, GA',
 'Swainton, NJ',
 'Swift Creek, VA',
 'Swiftwater, PA',
 'Sycamore, IL',
 'Syracuse, NY',
 'Tacoma, WA',
 'Tahlequah, OK',
 'Tahoe City, CA',
 'Talladega, AL',
 'Tallahassee, FL',
 'Tampa, FL',
 'Tarrytown, NY',
 'Taunton, MA',
 'Temecula, CA',
 'Tempe, AZ',
 'Terre Haute, IN',
 'Tewksbury, MA',
 'Texarkana, AR',
 'The Bronx, NY',
 'The Dalles, OR',
 'The Villages, FL',
 'The Woodlands, TX',
 'Thomaston, CT',
 'Thomasville, AL',
 'Thomasville, GA',
 'Thomasville, NC',
 'Thompson, CT',
 'Thornton, CO',
 'Thousand Oaks, CA',
 'Tiffin, OH',
 'Tifton, GA',
 'Timonium, MD',
 'Tinley Park, IL',
 'Tinton Falls, NJ',
 'Titusville, FL',
 'Titusville, NJ',
 'Toledo, OH',
 'Tolland, CT',
 'Toms River, NJ',
 'Tooele, UT',
 'Topeka, KS',
 'Topping, VA',
 'Torrance, CA',
 'Torrington, CT',
 'Totowa, NJ',
 'Towson, MD',
 'Tracy, CA',
 'Traverse City, MI',
 'Trenton, MI',
 'Trenton, NJ',
 'Trevose, PA',
 'Troutdale, OR',
 'Troy, AL',
 'Troy, MI',
 'Troy, NY',
 'Troy, OH',
 'Truckee, CA',
 'Trumbull, CT',
 'Trussville, AL',
 'Tucson, AZ',
 'Tukwila, WA',
 'Tulalip Bay, WA',
 'Tulare, CA',
 'Tullahoma, TN',
 'Tulsa, OK',
 'Tupelo, MS',
 'Turlock, CA',
 'Turner, OR',
 'Tuscaloosa, AL',
 'Tuthilltown, NY',
 'Twentynine Palms, CA',
 'Twin Falls, ID',
 'Tyler, TX',
 'Tysons Corner, VA',
 'Ukiah, CA',
 'Union, CT',
 'Union, NJ',
 'Uniontown, OH',
 'University Place, WA',
 'Upper Alton, IL',
 'Upper Saddle River, NJ',
 'Urbandale, IA',
 'Urbanna, VA',
 'Utica, MI',
 'Utica, NY',
 'Valdosta, GA',
 'Valencia, CA',
 'Valhalla, NY',
 'Valparaiso, IN',
 'Vancouver, WA',
 'Vandalia, OH',
 'Van Nuys, CA',
 'Venice, FL',
 'Ventura, CA',
 'Vernon, CT',
 'Vernon Hills, IL',
 'Vero Beach, FL',
 'Vicksburg, MS',
 'Victor, NY',
 'Victorville, CA',
 'Vienna, VA',
 'Villa Park, IL',
 'Vincennes, IN',
 'Vineland, NJ',
 'Virginia Beach, VA',
 'Volcano, HI',
 'Voluntown, CT',
 'Wading River, NY',
 'Wahiawā, HI',
 'Wailea, HI',
 'Wailuku, HI',
 'Wakefield, MA',
 'Wake Forest, NC',
 'Waldorf, MD',
 'Walla Walla, WA',
 'Wallingford, CT',
 'Wallops Island, VA',
 'Walnut Creek, CA',
 'Waltham, MA',
 'Warminster, PA',
 'Warner Robins, GA',
 'Warren, CT',
 'Warren, MI',
 'Warren, NJ',
 'Warren, OH',
 'Warrensburg, MO',
 'Warrensburg, NY',
 'Wasco, CA',
 'Washington, CT',
 'Washington, DC',
 'Washington, MI',
 'Washington, UT',
 'Washougal, WA',
 'Watchung, NJ',
 'Waterbury, CT',
 'Waterbury, VT',
 'Waterford, CT',
 'Waterford, MI',
 'Waterloo, IA',
 'Watertown, CT',
 'Watertown, NY',
 'Watertown, SD',
 'Waterville, ME',
 'Watsonville, CA',
 'Waukesha, WI',
 'Waunakee, WI',
 'Wausau, WI',
 'Wautoma, WI',
 'Wauwatosa, WI',
 'Wayne, NJ',
 'Wayne, PA',
 'Webster, MA',
 'Weirton Heights, WV',
 'Weirton, WV',
 'Welches, OR',
 'Wellesley, MA',
 'Wellington, FL',
 'Wells, ME',
 'Wenatchee, WA',
 'Weslaco, TX',
 'Wesley Chapel, FL',
 'West Bend, WI',
 'Westborough, MA',
 'Westbrook, CT',
 'West Caldwell, NJ',
 'Westchester, NY',
 'West Chester, PA',
 'West Dundee, IL',
 'Westerly, RI',
 'West Fargo, ND',
 'Westfield, IN',
 'Westfield, MA',
 'West Gulfport, MS',
 'West Hartford, CT',
 'West Haven, CT',
 'West Hollywood, CA',
 'West Lafayette, IN',
 'Westlake, OH',
 'Westlake, TX',
 'Westlake Village, CA',
 'Westland, MI',
 'West Milford, NJ',
 'Westminster, CO',
 'Westminster, MD',
 'West Odessa, TX',
 'Weston, CT',
 'Weston, FL',
 'Weston, MA',
 'West Orange, NJ',
 'West Palm Beach, FL',
 'Westport, CT',
 'West Sacramento, CA',
 'West Salem, WI',
 'West Springfield, MA',
 'West Torrington, CT',
 'West Valley City, UT',
 'Westwood, MA',
 'Wethersfield, CT',
 'Wexford, PA',
 'Wheaton, IL',
 'Wheeling, WV',
 'Whippany, NJ',
 'White Lake, MI',
 'White Marsh, MD',
 'White Plains, NY',
 'White River Junction, VT',
 'White Settlement, TX',
 'Whitestown, IN',
 'White, TX',
 'Whitney, TX',
 'Wichita, KS',
 'Wildomar, CA',
 'Wildwood, FL',
 'Wildwood, MO',
 'Williams Bay, WI',
 'Williamsburg, VA',
 'Williamsport, PA',
 'Williamstown, NJ',
 'Willington, CT',
 'Williston, ND',
 'Willis, TX',
 'Willmar, MN',
 'Wilmette, IL',
 'Wilmington, DE',
 'Wilmington, MA',
 'Wilmington, NC',
 'Wilmington, OH',
 'Wilmont, MN',
 'Wilson, NC',
 'Wilton, CT',
 'Winchester, CT',
 'Winchester, KY',
 'Winchester, VA',
 'Winder, GA',
 'Windham, CT',
 'Windsor, CA',
 'Windsor, CT',
 'Windsor Locks, CT',
 'Winona, MN',
 'Winston Salem, NC',
 'Winter Garden, FL',
 'Winter Gardens, CA',
 'Winter Haven, FL',
 'Winter Park, FL',
 'Wisconsin Dells, WI',
 'Wisconsin Rapids, WI',
 'Woburn, MA',
 'Wolcott, CT',
 'Woodbridge, CT',
 'Woodbridge, NJ',
 'Woodbridge, VA',
 'Woodburn, OR',
 'Woodbury, CT',
 'Woodbury, MN',
 'Woodcliff Lake, NJ',
 'Woodinville, WA',
 'Woodland, CA',
 'Woodland Hills, CA',
 'Woodmere, OH',
 'Woodstock, CT',
 'Woodstock, IL',
 'Woonsocket, RI',
 'Wooster, OH',
 'Worcester, MA',
 'Worthington, OH',
 'Wrentham, MA',
 'Wyandotte, MI',
 'Wyoming, MI',
 'Wyomissing, PA',
 'Xenia, OH',
 'Yakima, WA',
 'Yemassee, SC',
 'Yonkers, NY',
 'York, PA',
 'Youngstown, OH',
 'Ypsilanti, MI',
 'Yuba City, CA',
 'Yucca Valley, CA',
 'Yukon, OK',
 'Yuma, AZ',
 'Zachary, LA',
 'Zanesville, OH',
 'Zionsville, IN']